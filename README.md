...
caller stub 和可复用 workflow 都声明了 `permissions: contents: write` + `pull-requests: read`（删除分支引用需要 contents: write；查询已合并/已关闭 PR 需要 pull-requests: read）。若仓库 Settings → Actions → General → Workflow permissions 设为 "Read-only"，需改为 "Read and write permissions" 或保持 workflow 内显式声明。
